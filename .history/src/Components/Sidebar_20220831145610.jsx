...
function Sidebar(props) {
  return (
    <div className={` ${props.sidebar? ' md:w-72' : ' w-0'} pt-20 flex shadow-2xl flex-col items-start  h-full transform transition-width duration-500 ease-out  font-semibold `}>
        {array.map((item ,index) => <div className=' hover:bg-yellow-200 w-full rounded-3xl'><button className='py-4 px-5 hover:bg-yellow-200'>{item.icon} {props.sidebar && item.text }</button></div>)}
    </div>
  )
}
